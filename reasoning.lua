...
function M.train(model, opt, batches, val_batches, optim_state, dataloader)
    local params, grad_params = model_utils.combine_all_parameters(model.emb, model.soft_att_lstm, model.lstm, model.softmax)
    local clones = {}
    ...
    local function feval(params_, update)
        ...
        if update then
            ...
            for t = reason_len, 1, -1 do
                _, _, dreason_c[t - 1], dreason_h[t - 1] = unpack(clones.soft_att_lstm[t]:
                    backward({fc7_images, att_seq, reason_c[t - 1], reason_h[t - 1]},
...
function M.create_model(opt)
    ...
    model.soft_att_lstm = M.soft_att_lstm_concat(opt)
