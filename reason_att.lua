...
function M.train(model, opt, batches, val_batches, optim_state, dataloader)
    ...
    local function feval(params_, update)
        ...
        local reason_pool
        if opt.use_noun then
            reason_pool = model.pooling:forward(reason_pred_mat)
            print(reason_pool:size(), noun_list:size())
        ...
        if update then
            local dreason_pred
            if opt.use_noun then
                dreason_pred = model.reason_criterion:backward(reason_pool, noun_list)
                dreason_pred = model.pooling:backward(reason_pred_mat)
            end
