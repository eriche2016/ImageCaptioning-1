    ...
    local function feval(params_, update)
        ...
        for t = 1, seq_len do
            ...
            lstm_c[t], lstm_h[t] = unpack(clones.lstm[t]:
                forward{embeddings[t], reason_pred_mat, lstm_c[t - 1], lstm_h[t - 1]})
        ...
        if update then
            ...
            for t = seq_len, 1, -1 do
                ...
                dembeddings[t], doutput_t, dlstm_c[t - 1], dlstm_h[t - 1] = unpack(clones.lstm[t]:
                    backward({embeddings[t], reason_pred_mat, lstm_c[t - 1], lstm_h[t - 1]},
                    {dlstm_c[t], dlstm_h[t]}))
                -- dreason_h_att:add(doutput_t)
                dreason_pred:add(doutput_t)
        ...
        for i = 1, #batches do
            ...
            if i == 1 or i % opt.eval_period == 0 then
                ...
                while j1 <= #dataloader.val_set do
                    ...
                    local reason_len = opt.reason_step
                    local reason_preds = {}
                    local reason_pred_mat = torch.CudaTensor(att_seq:size()[1], opt.reason_step, opt.word_cnt)
                    local seq_len = max_t
