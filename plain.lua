...
function M.soft_att_lstm_concat_nox(opt)
    ...
    return nn.gModule({prev_c, prev_h}, {next_c, next_h})
end
    ...
    local function feval(params_, update)
        ...
        for t = 1, reason_len do
            reason_c[t], reason_h[t] = unpack(clones.soft_att_lstm[t]:
                forward{reason_c[t - 1], reason_h[t - 1]})
        ...
        if update then
            ...
            for t = reason_len, 1, -1 do
                ...
                dreason_c[t - 1], dreason_h[t - 1] = unpack(clones.soft_att_lstm[t]:
                    backward({reason_c[t - 1], reason_h[t - 1]},
